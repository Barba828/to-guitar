...
const getTones = (zeros: ToneType[] = DEFAULT_TUNE) => {
	const zeroGrades = transNotes(zeros)
...
const transBoard = (zeroTones: ToneType[] = DEFAULT_TUNE, GradeLength: number = GRADE_NUMS) => {
	const zeroGrades = transNotes(zeroTones)
 ...
 */
const transChordTaps = (tones: ToneType[], points: Point[][] = transBoard(), fingerSpan: number = FINGER_GRADE_NUMS) => {
	const chords = transNotes(tones)
 ...
 * @returns Note
 */
const transNote: (x: ToneType) => Note = (x: ToneType) => {
	return isNote(x)
		? x
		: isNoteFalling(x)
...
const transNotes: (xs: ToneType[]) => Note[] = (xs: ToneType[]) => {
...
const transScaleDegree = (scale: ToneType) => {
	...
	degrees.forEach((degree, index) => {
		degree.chord = chordScale.map((scale) => degrees[(index + scale) % degreeLength].note)
...
const transChordDegree = (chords: ToneType[], calGrades: number) => {
	const chordNotes = transNotes(chords)
	const calNotes = chordNotes
		.map((note) => NOTE_LIST.indexOf(note))
		.map((tone) => tone + calGrades)
